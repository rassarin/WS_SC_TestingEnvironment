    ...
    character(len=35):: SZ,fileName,argDisAg,argExportWTD,argCopyWTD,WTHdir,argDSSATrun,argDelWTD
    !KH: Add for RunQQ
    character(len=35):: runCmd 
    ...
    character(len=8) :: x1
    logical:: EXISTS
    character(len=13) :: file_name1,file_name2
    ...
    write(6,'(a,1x,a)')'Opening historical climate data:',trim(stationName)//'.WTD'
    ...
    open(unit=11,file='BASE0000.WTD')
    write(6,'(a,1x,a)')'Creating base weather file:','BASE0000.WTD'
    ...
    close(11)
    write(6,'(a30)') SZ
    argPRM=trim(stationName)//'.CLI'//' '//trim(stationName)//'.WTD' !//' '//trim(SZ)
    ...
    target_loop: do ii=1,10
        ...
        argDisAg=trim(stationName)//'.PRM'//' '//trim(stationName)//'.MTH'//' '//trim(SZ)
        ...
        MainDO_processing: do i=1,nRealz2
            ...
            open(unit=99+i,file=fname(i)(1:8)//'.WTD',iostat=iost) !predicted file
            fnum=99+i; call Errflg(fnum,iost,stationName,fname(i),expName)

            write(6,'(a,1x,a)')'Processing merged',trim(fname(i)(1:8))//'.WTDE'
            open(unit=999+i,file=fname(i)(1:8)//'.WTDE',iostat=iost) !merged file
        ...
        if(ii .eq. 1) then  !copy the original 10 *.WTDE to temporary files
            do j=1,nRealz2
                ...
                file_name1=trim(stationname)//trim(x1)//'.WTDE'
                file_name2='temp'//trim(x1)//'.WTDE'
            ...
            do j=1,nRealz2
                ...
                file_name2=trim(stationname)//trim(x1)//'.WTDE'
